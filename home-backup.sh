...
if [ -d /home/patrick/Photos/.NeverDelete ]; then      # the external photos drive is plugged in
	echo "Pruning backup dir ..."; sleep 2; rsync -avhHSP --delete --ignore-errors --delete-delay --exclude ".thumbnails" --exclude ".gvfs" --exclude ".cache" --exclude ".temp" --exclude ".config/google-chrome/Default/Application Cache/" --exclude ".local/share/Trash" --exclude ".dropbox-dist/dropbox-lnx*" --exclude ".dropbox/" --exclude ".dropbox.cache" --exclude ".config/google-chrome" --exclude ".PyCharmCE2*" --exclude ".local/share/gvfs-metadata" --exclude ".config/libreoffice" --log-file=/home/patrick/Desktop/home_backup.log -e ssh /home/patrick patrick@file-server:/media/Backup/Backups
